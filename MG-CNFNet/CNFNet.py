...
class HinEncoder(nn.Module):
    def __init__(self, n_feat, kernel_size, bias, scale_unetfeats, relu_slope):
        ...
        # scale_unetfeats跳跃连接   for _ in range(n)中，_ 是占位符， 表示不在意变量的值 只是用于循环遍历n次。

        self.encoder_level1 = [UNetConvBlock(n_feat, n_feat, relu_slope, use_HIN=True)]
        self.encoder_level2 = [UNetConvBlock(n_feat, n_feat + scale_unetfeats, relu_slope, use_HIN=True)]
        self.encoder_level3 = [UNetConvBlock(n_feat + scale_unetfeats, n_feat + scale_unetfeats * 2, relu_slope, use_HIN=True)]
        self.encoder_level4 = [UNetConvBlock(n_feat + scale_unetfeats * 2, n_feat + scale_unetfeats * 3, relu_slope, use_HIN=True)]
        # self.encoder_level5 = [UNetConvBlock(n_feat+scale_unetfeats*3, n_feat+scale_unetfeats*4, relu_slope, downsample=False, use_HIN=True)]
        # 顺序连接模型
        self.encoder_level1 = nn.Sequential(*self.encoder_level1)
        self.encoder_level2 = nn.Sequential(*self.encoder_level2)
        self.encoder_level3 = nn.Sequential(*self.encoder_level3)
        ...
        self.down12 = conv_down(n_feat, n_feat, bias=False)
        self.down23 = conv_down(n_feat + scale_unetfeats, n_feat + scale_unetfeats, bias=False)
        self.down34 = conv_down(n_feat + scale_unetfeats * 2, n_feat + scale_unetfeats * 2, bias=False)
...
class ResDecoder(nn.Module):
    def __init__(self, n_feat, kernel_size, relu_slope, bias, scale_unetfeats):
        super(ResDecoder, self).__init__()

        self.decoder_level1 = [UNetConvBlock(n_feat + scale_unetfeats, n_feat, relu_slope, use_HIN=False)]
        self.decoder_level2 = [UNetConvBlock(n_feat + scale_unetfeats * 2, n_feat + scale_unetfeats, relu_slope, use_HIN=False)]
        self.decoder_level3 = [UNetConvBlock(n_feat + (scale_unetfeats * 3), n_feat + (scale_unetfeats * 2), relu_slope, use_HIN=False)]
    ...
    def forward(self, outs):
        ...
        x = self.skip_attn3(enc3) + self.up43(enc4)  # [16,224,16,16]
        dec3 = self.decoder_level3(x)  # 16,176,16,16

        x = self.skip_attn2(enc2) + self.up32(dec3)  # 16,176,32,32
        dec2 = self.decoder_level2(x)  # 16,128,32,32

        x = self.skip_attn1(enc1) + self.up21(dec2)  # 16,128,64,64
        dec1 = self.decoder_level1(x)  # 16,80,64,64
...
class SB(nn.Module):
    def __init__(self, in_channels):
        ...
        '''
        self.conv1_1 = nn.Sequential(nn.Conv2d(in_channels=in_channels,out_channels=features,kernel_size=kernel_size,padding=padding,groups=groups,bias=False),nn.BatchNorm2d(features),nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        res = self.conv1_4(res)
        res = self.conv1_5(res)
        res = self.conv1_6(res)
        res = self.conv1_7(res)
        res = self.conv1_8(res)
        res = self.conv1_9(res)
        res = self.conv1_10(res)
        res = self.conv1_11(res)
        res = self.conv1_12(res)
        return res

##########################################################################
class CNFNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=80, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3, relu_slope=0.2, reduction=4, bias=False):
        ...
        self.SB_block = SB(in_c)

        self.stage1_encoder = HinEncoder(n_feat, kernel_size, bias, scale_unetfeats, relu_slope)
